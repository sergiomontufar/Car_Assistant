...
def main() -> None:
    ...
    def on_speech_detected(raw_bytes: bytes) -> None:
        nonlocal last_bot_response, last_play_started_ts, is_awake
        ...
        if not is_awake:
            # Require wake word "computer" to start listening
            if lower_text.startswith("computer"):
                is_awake = True
                recognized_text = recognized_text[len("computer"):].strip()
                if not recognized_text:
                    return
            else:
                return
        else:
            # If awake and hears "stop" at start, go back to sleep
        ...
        # Switch system prompt language and enforce concise replies
